...
import { request, subscribe } from './platform.js';
...
import { practiceType } from './routes.js';
import { compareOutput, explainError } from './feedback.js';

const mac = /Mac|iPhone|iPad/.test(navigator.platform);

export default function WorkspaceRoute() {
  const { lessonId } = useParams(), lesson = lessons.find(l => l.id === lessonId);
...
function LessonPanel({ lesson }) {
  ...
  function applyResult(packet, history = false) {
    ...
    setFocusPane('output');
    if (packet.result.error) { setPhase('error'); setFeedback(packet.result.stage === 'compile' ? 'Fix the compiler error in `Student.java`. Details are also in VS Code’s Problems panel.' : packet.result.stage==='checks'?'Practice the required Java construct. Read the checks below.':'The program stopped. Read the details below.'); }
  ...
  useEffect(() => {
    ...
    request('lesson.open', { lessonId: lesson.id }).then(data => { if (!alive.current) return; setFile(data.file); if (data.result) callbacks.current.applyResult(data.result, true); }).catch(e => { if(alive.current) setSetupError(e.message); });
    const shortcut = e => { if (e.key === 'Enter' && (mac ? e.metaKey : e.ctrlKey)) { e.preventDefault(); e.target.id === 'custom-input' ? callbacks.current.tryInput() : callbacks.current.run(); } };
  ...
  const action = async (method, params = {}) => {
    try { const data = await request(method, { lessonId: lesson.id, ...params }); if (method === 'lesson.run') callbacks.current.applyResult(data); return data; }
    catch(e) { if(alive.current) { setBusy(false); setPhase('error'); setFeedback(e.message); } }
  };
  const run = () => { if (busy || setupError) return; setBusy(true); return action('lesson.run', { controlHz }); };
  const tryInput = () => { if (busy || setupError) return; setBusy(true); return action('lesson.run', { controlHz, input: customInput }); };
  ...
  return <div ref={shell} className="lesson-panel">
    ...
    {setupError && <section className="setup-error" role="alert"><p>{setupError}</p><button onClick={() => action('workspace.open')}>Open learning folder</button><button onClick={() => window.location.reload()}>Retry setup</button></section>}
    <section className="lesson-instruction focus-region" aria-label="Current task" {...activeRegion('activity')}><div className="region-heading"><strong>Task</strong><span>{paneFocused&&focusPane==='activity'?'In focus':'Practice'}</span></div><p><CodeText>{lesson.task}</CodeText></p>
      ...
      {hintLevel>1&&<div className="hint-example"><p><CodeText>{concept}</CodeText></p>{lesson.example&&<pre><code>{lesson.example}</code></pre>}</div>}
    ...
    <footer className="lesson-tools"><span><code>{file}</code></span><button disabled={busy} className="text-button" onClick={()=>action('lesson.restore')}>Restore starter</button></footer>
